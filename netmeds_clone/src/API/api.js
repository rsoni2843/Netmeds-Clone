...
function getHome(){
  return axios.get(`https://api.allorigins.win/raw?url=${encodeURIComponent(`https://pharmeasy.in/apt-api/loyalty/fetchData?custom_token=home`)}`)
}

function getOffer() {
  return axios.get(`https://api.allorigins.win/raw?url=${encodeURIComponent(paymentOfferAPI)}`);
}
function getTrending() {
  return axios.get(trendingAPI);
}
function getPrivate() {
  return axios.get(`https://api.allorigins.win/raw?url=${encodeURIComponent(privateLabelAPI)}`);
}
function getNutrition(){
  return axios.get(`https://api.allorigins.win/raw?url=${encodeURIComponent(originNutrition)}`)
}
