...
function Footer() {
  return (
    <Box bg={"#fff"}>
      ...
      >
        <Hide below="sm">
          <Box w={"20%"}>
            ...
            {company.map((item, i) => {
              return (
                <Text fontWeight={400} fontSize={12} lineHeight={8} key={i}>
                  {item.page}
                </Text>
              );
            })}
        ...
        <Hide below="md">
          <Box w={"20%"}>
            ...
            {shopping.map((item, i) => {
              return (
                <Text fontSize={12} lineHeight={8} key={i}>
                  {item.page}
                </Text>
              );
            })}
        ...
        <Box w={{ md: "20%", base: "60%" }}>
          ...
          <Flex mt={3} w={"100%"} justifyContent={"space-around"}>
            <Link
              w={"40%"}
              href="https://play.google.com/store/apps/details?id=com.NetmedsMarketplace.Netmeds&hl=en"
            >
              <Image
                src={
                  "https://www.netmeds.com/assets/gloryweb/images/icons/play_store.png"
                }
              />
            </Link>
            <Link
              w={"40%"}
              href="https://apps.apple.com/in/app/netmeds-india-ki-pharmacy/id1011070011"
            >
              <Image
                src={
                  "https://www.netmeds.com/assets/gloryweb/images/icons/app_store.png"
                }
              />
            </Link>
          </Flex>
      ...
      <Show below="sm">
        <Text p={4} fontSize={10} color={"grey"}>
