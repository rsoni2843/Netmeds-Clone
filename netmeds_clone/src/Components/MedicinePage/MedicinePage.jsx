import { Box, Button, Flex, Image, Skeleton, Stack, Tag, TagLabel, Text } from '@chakra-ui/react';
...
function MedicinePage() {
  ...
  return (
    <Box w={{base:'96%',md:'75%'}} m={'auto'} bg={'white'}>
      <Text fontSize={14} p={2} mt={3} color={'grey'} textAlign={'left'}>Showing results for <b>{name}</b></Text>
      <Stack  w={'100%'} m={'auto'} direction={{base:'column',md:'row'}}>
          <Box p={4} w={{base:'100%',md:'70%'}}>
            {med?.map((item,i)=>{
           return (<Box p={3} bg='gray.50' key={item[8]} textAlign={'left'} rounded={'lg'} boxShadow='md' w={'95%'} m={'auto'} mt={4} >
              <Flex direction={{base:'column',md:'row'}} minH={'130px'}>
                  <Box w={{base:'98%',md:'25%'}}>
                    {!item[11]?
                      <Image w={'100px'} m={'auto'} src={`https://www.netmeds.com/images/product-v1/75x75/formulation_based/tablets.jpg`}/>:
                      <Image w={'100px'} m={'auto'} src={`https://res.sastasundar.com/incom/images/product/thumb/${item[11]}`}/>
                  }
                    
                  </Box>
                  <Box lineHeight={8} w={{base:'98%',md:'50%'}}>
                    <Text fontSize={18} fontWeight={600}>{item[10]}</Text>
                    <Flex mt={3}>
                  ...
                  <Box display={'flex'}
                      ...
                      alignItems={'center'} w={{base:'98%',md:'25%'}}>
                  <Link to={'login'}>
                    {item[66]==="Y"?
